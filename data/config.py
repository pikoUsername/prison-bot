...
class Config:
    def __init__(self, fp, lazy=True, setup_items=False, **defaults):
        self.fp = fp
    ...
    def load(self, cache=False):
        ...
        if cache: self._cache = data
        return data

    def copy(self):
        if self._cache and not self.defaults: return self._cache.copy()
        if self.setup_items: return self

    def __getitem__(self, key):
        if self._cache: return self._cache[key]
        if self.defaults: return self.defaults[key]
